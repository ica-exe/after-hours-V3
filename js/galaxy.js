...
class SunflowerParticleVisual {
  ...
  createParticles() {
    const palette = {
      core: ['#2F1608', '#45210A', '#5A2B0C', '#6B3510'], mid: ['#71370D', '#8A460F', '#A95710', '#C46A14'],
      rim: ['#B75B10', '#CC7415', '#D98A1A', '#E3A12B'], base: ['#C96A11', '#D87A13', '#E4941B', '#EAA827'],
      body: ['#E6A51F', '#EEB82E', '#F3C43D', '#F6CC4A', '#E9AF27', '#D99518'],
      tip: ['#F7D65A', '#F9DD70', '#F4C84A', '#F8D95F'],
      sunflower: ['#DEA01D', '#EAB52F', '#F1C240', '#F5CB50', '#E3A422', '#F7D65C'],
    ...
    const add = (type, data) => {
      ...
      this.particles.push({
        ...
        heartAngle: Math.random() * Math.PI * 2,
        heartScale: Math.sqrt(Math.random()),
        ...data
      });
    };
  ...
  frame(now) {
    ...
    if (this.state === 'GATHERING') {
      ...
      this.heartProgress = 0;
      if (this.stateTimer >= 3) { this.state = 'HEART_FORMING'; this.stateTimer = 0; }
    } else if (this.state === 'HEART_FORMING') {
      const progress = Math.min(1, this.stateTimer / 1.2);
      this.formationProgress = 1;
      this.bloomProgress = 1;
      this.heartProgress = progress * progress * (3 - 2 * progress);
      if (progress === 1) { this.state = 'HEART_HOLDING'; this.stateTimer = 0; }
    } else if (this.state === 'HEART_HOLDING') {
      this.formationProgress = 1;
      this.bloomProgress = 1;
      this.heartProgress = 1;
      if (this.stateTimer >= 2) { this.state = 'REFORMING'; this.stateTimer = 0; }
    } else if (this.state === 'REFORMING') {
      const progress = Math.min(1, this.stateTimer / 1.1);
      const ease = progress * progress * (3 - 2 * progress);
      this.formationProgress = 1;
      this.bloomProgress = 1 - ease;
      this.heartProgress = 1 - ease;
    ...
    if (this.visible) {
      ...
      this.particles.forEach(p => {
        ...
        if (this.heartProgress > 0) {
          const t = p.heartAngle;
          const heartX = 16 * Math.pow(Math.sin(t), 3);
          const heartY = -(13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t));
          const heartSize = r * .055 * p.heartScale * sceneScale;
          const targetX = cX + heartX * heartSize;
          const targetY = cY + heartY * heartSize;
          endX += (targetX - endX) * this.heartProgress;
          endY += (targetY - endY) * this.heartProgress;
        }
...
class GalaxyController {
  ...
  animate() {
    ...
    const shouldFrameBloom = this.sunflowerVisual && !this.sunflowerViewMoved &&
      ['BLOOMING', 'HOLDING', 'HEART_FORMING', 'HEART_HOLDING', 'REFORMING'].includes(this.sunflowerVisual.state);
    if (shouldFrameBloom) {
