...
/*
 * Canvas-only visual replacement for the central heart.  It deliberately owns
 * no timeline or input: GalaxyController remains the source of truth for the
 * original heart's appearance, click, explosion, and transition lifecycle.
 */
class SunflowerParticleVisual {
  constructor(container) {
    this.canvas = document.createElement('canvas');
    this.canvas.className = 'sunflower-particle-canvas';
  ...
  resize() {
    const flowerSize = Math.min(560, Math.max(280, Math.min(window.innerWidth, window.innerHeight) * 0.58));
    // High-DPI canvas rendering is expensive for thousands of glowing paths;
    // 1.5 retains crisp wisps while substantially reducing fill-rate on phones.
    const dpr = Math.min(window.devicePixelRatio || 1, 1.5);
    this.width = window.innerWidth;
    this.height = window.innerHeight;
    this.size = flowerSize;
    this.radius = flowerSize * 0.39;
  ...
  createParticles() {
    const palette = {
      core: ['#5C2E0E', '#7C243C', '#5B2C83', '#14532D'], mid: ['#C04A0B', '#C2185B', '#7E57C2', '#15803D'],
      rim: ['#FF6B35', '#FF9800', '#FFC300', '#22C55E', '#38BDF8', '#A855F7'], base: ['#E53935', '#F4511E', '#F59E0B', '#84CC16', '#0EA5E9', '#9333EA'],
      body: ['#FF4D6D', '#FF6B35', '#FF9800', '#FFC300', '#84CC16', '#22C55E', '#38BDF8', '#6366F1', '#A855F7', '#EC4899'],
      tip: ['#FF8FAB', '#FFD166', '#FDE047', '#BEF264', '#67E8F9', '#A5B4FC', '#D8B4FE'],
      sunflower: ['#FF4D6D', '#FF6B35', '#FF9800', '#FFC300', '#84CC16', '#22C55E', '#38BDF8', '#6366F1', '#A855F7', '#EC4899'],
      budCore: ['#0B3D20', '#14532D', '#4A1942', '#312E81'],
      budBody: ['#15803D', '#22C55E', '#4ADE80', '#0EA5E9', '#8B5CF6', '#EC4899'],
      budTip: ['#84CC16', '#A3E635', '#FDE047', '#FB923C', '#67E8F9', '#C084FC']
    };
    const pick = list => list[Math.floor(Math.random() * list.length)];
    const add = (type, data) => {
      // Organic, uneven cloud around the flower: this deliberately avoids
      // screen-aligned rectangular targets during dispersion or gathering.
      const scatterAngle = Math.random() * Math.PI * 2;
      const scatterDistance = this.radius * (.12 + Math.pow(Math.random(), 2.2) * .82);
      this.particles.push({
        type, phase: Math.random() * Math.PI * 2, speed: 1 + Math.random() * 2.5,
        scatterX: Math.cos(scatterAngle) * scatterDistance,
        scatterY: Math.sin(scatterAngle) * scatterDistance * (.55 + Math.random() * .72),
        initialScatterX: (Math.random() - .5) * this.width * 1.25,
        initialScatterY: (Math.random() - .5) * this.height * 1.1,
        rearrangeAngle: Math.random() * Math.PI * 2,
        rearrangeDistance: this.radius * (.045 + Math.random() * .16),
        ...data
      });
    };
    const goldenAngle = Math.PI * (3 - Math.sqrt(5));

    // Fibonacci seed disc from the supplied animation.
    for (let i = 0; i < 4000; i++) {
      const r = Math.sqrt(i / 4000);
      const accent = Math.random();
      const color = r < .26 ? pick(palette.core) : r < .72 ? pick(palette.mid) : pick(palette.rim);
      add('seed', { r, angle: i * goldenAngle, color, budColor: pick(palette.budCore), baseSize: .28 + Math.random() * .72 + (1 - r) * .14 });
    }
    [
      { count: 16, inR: .38, outR: .66, width: .18, sag: .015, stagger: .28, samples: 82 },
      { count: 22, inR: .40, outR: .90, width: .22, sag: .030, stagger: .14, samples: 104 },
      { count: 28, inR: .42, outR: 1.12, width: .25, sag: .045, stagger: 0, samples: 126 }
    ].forEach((layer, layerIndex) => {
      for (let petal = 0; petal < layer.count; petal++) {
        const angle = petal * Math.PI * 2 / layer.count + layerIndex * Math.PI / layer.count;
        // Shared values give each closed bract one coherent organic shape,
        // rather than a set of independent upright particle spikes.
        const budHeight = .76 + Math.random() * .28;
        const budLean = (Math.random() - .5) * .34;
        const budBulge = .88 + Math.random() * .22;
        const bloomLength = .91 + Math.random() * .16;
        for (let point = 0; point < layer.samples; point++) {
          const u = .025 + .955 * Math.sqrt(point / (layer.samples - 1));
          // Broad ray petals: narrow at the seed head, widest in the outer
          // half, and naturally pointed without turning into sharp spikes.
          const width = Math.sin(Math.PI * Math.pow(u, .78)) * (1.12 - u * .16);
          const v = ((point % 9) / 8 - .5) * width + (Math.random() - .5) * .055;
          const accent = Math.random();
          const edge = Math.abs(v) > width * .52;
          const color = u < .32 ? pick(palette.base) : u > .76 ? pick(palette.tip) : pick(palette.sunflower);
          add('petal', {
            layer, angle, u, v, layerIndex, color, edge,
            budColor: u > .76 ? pick(palette.budTip) : u > .36 ? pick(palette.budBody) : pick(palette.budCore), sparkle: edge && Math.random() < .10,
            budHeight, budLean, budBulge, bloomLength,
            baseSize: (edge ? .46 : .34) + Math.random() * .72, twist: Math.random() * Math.PI * 2
          });
        }
      }
    });
    // The flower itself supplies the visual detail; no surrounding glitter field.
  }

  setVisible(visible, x, y, sceneScale, sceneYaw, sceneTilt) {
    // Begin gathering only when the original heart first becomes visible.
    ...
    this.sceneTilt = sceneTilt;
    this.canvas.style.opacity = visible ? '1' : '0';
  }

  frame(now) {
    const dt = Math.min((now - this.lastTime) / 1000, .08);
    this.lastTime = now;
    this.rotation += .06 * dt;
    this.stateTimer += dt;
    if (this.state === 'GATHERING') {
      // First appearance: let the loose green particles gather into the bud
      // gradually before the existing bloom sequence begins.
      const progress = Math.min(1, this.stateTimer / 3);
      this.formationProgress = progress * progress * (3 - 2 * progress);
      this.bloomProgress = 0;
      if (progress === 1) { this.isFirstGathering = false; this.state = 'CLOSED'; this.stateTimer = 0; }
    } else if (this.state === 'CLOSED') {
      this.formationProgress = 1;
      this.bloomProgress = 0;
      // On the first cycle, let the existing language-word reveal finish
      // before opening the flower. Later cycles keep a one-second bud pause.
      const wordsReady = !this.isFirstCycle || (this.owner && this.owner.stage >= 5);
      if (wordsReady && this.stateTimer >= 1) {
        this.isFirstCycle = false;
        this.state = 'BLOOMING';
        this.stateTimer = 0;
      }
    } else if (this.state === 'BLOOMING') {
      const progress = Math.min(1, this.stateTimer / 1);
      this.formationProgress = 1;
      this.bloomProgress = progress * progress * (3 - 2 * progress);
      if (progress === 1) { this.state = 'HOLDING'; this.stateTimer = 0; }
    } else if (this.state === 'HOLDING') {
      this.formationProgress = 1;
      this.bloomProgress = 1;
      if (this.stateTimer >= 3) { this.state = 'DISPERSING'; this.stateTimer = 0; }
    } else if (this.state === 'DISPERSING') {
      const progress = Math.min(1, this.stateTimer / 5);
      // The bloom stays together and organically rearranges rather than
      // exploding outward into a cloud.
      this.formationProgress = 1;
      this.bloomProgress = 1;
      if (progress === 1) { this.state = 'REFORMING'; this.stateTimer = 0; }
    } else if (this.state === 'REFORMING') {
      const progress = Math.min(1, this.stateTimer / .5);
      const ease = progress * progress * (3 - 2 * progress);
      this.formationProgress = 1;
      this.bloomProgress = 1 - ease;
      if (progress === 1) { this.state = 'CLOSED'; this.stateTimer = 0; }
    }
    const ctx = this.ctx, cX = this.originX || this.width / 2, cY = this.originY || this.height / 2, r = this.radius, sceneScale = this.sceneScale || 1;
    ctx.clearRect(0, 0, this.width, this.height);
    if (this.visible) {
      ctx.save();
      ctx.globalCompositeOperation = 'lighter';
      // The unopened bud has no exposed seed disk. A small number of loose
      // botanical wisps circle the closed bracts instead of forming a dome.
      if (this.bloomProgress < .08 && this.formationProgress > .05) {
        const wispColors = ['#14532D', '#15803D', '#22C55E', '#84CC16', '#A3E635'];
        const wispAlpha = this.formationProgress * .48;
        for (let i = 0; i < 15; i++) {
          // Faster than the flower's slow clockwise turn, while remaining a
          // calm botanical orbit rather than a frantic spinning ring.
          const phase = this.rotation * (3.4 + i * .035) + i * (Math.PI * 2 / 15);
          const rx = r * (.17 + (i % 4) * .028) * sceneScale;
          const ry = r * (.065 + (i % 3) * .018) * sceneScale;
          const start = phase - (.62 + (i % 3) * .10);
          const end = phase + (.75 + (i % 2) * .14);
          ctx.globalAlpha = wispAlpha * (.44 + (i % 5) * .08);
          ctx.strokeStyle = wispColors[i % wispColors.length];
          ctx.lineWidth = .35 + (i % 3) * .18;
          ctx.beginPath();
          ctx.ellipse(cX, cY + r * .10 * sceneScale, rx, ry, phase * .18, start, end);
          ctx.stroke();
        }
      }
      this.ambient.forEach(p => {
        p.phase += dt * 1.5;
        ctx.globalAlpha = p.alpha * (.6 + .4 * Math.sin(p.phase));
        ctx.fillStyle = p.color;
        ctx.beginPath(); ctx.arc(cX + p.x * r * sceneScale, cY + p.y * r * sceneScale, p.size * sceneScale, 0, Math.PI * 2); ctx.fill();
      });
      const render = [];
      this.particles.forEach(p => {
        p.phase += p.speed * dt;
        // Hide the full seed field while the bud is closed; it is revealed as
        // part of the existing bloom rather than appearing as a green disk.
        if (p.type === 'seed' && this.bloomProgress < .08) return;
        let x, y, z;
        if (p.type === 'seed') {
          const isBud = this.bloomProgress < .08;
          const localR = p.r * r * (isBud ? .16 : (.23 + .22 * this.bloomProgress));
          const a = p.angle + this.rotation;
          x = localR * Math.cos(a);
          y = localR * Math.sin(a) * (isBud ? .70 : 1);
          z = (1 - p.r * p.r) * r * (isBud ? .09 : (.16 - .1 * this.bloomProgress));
        } else {
          const layerBloom = Math.max(0, Math.min(1, (this.bloomProgress - p.layer.stagger) / (1 - p.layer.stagger)));
          const easeBloom = layerBloom * layerBloom * (3 - 2 * layerBloom);
          // Closed-stage target: a rounded, overlapping ring of botanical
          // bracts, widest through the lower-middle and tapered at its tip.
          const bractBody = Math.sin(Math.PI * Math.pow(p.u, .82));
          const closedR = (.075 + (bractBody * .125 + p.u * (.055 + p.layerIndex * .016)) * p.budBulge) * r;
          const openR = (p.layer.inR + p.u * (p.layer.outR - p.layer.inR) * p.bloomLength) * r;
          const radial = closedR + (openR - closedR) * easeBloom;
          const tangent = p.v * p.layer.width * r * easeBloom;
          const a = p.angle + tangent / Math.max(radial, 1) + Math.sin(p.u * Math.PI * 2 + p.twist) * .02 * p.u * easeBloom + p.budLean * p.u * p.u * (1 - easeBloom) + this.rotation;
          const closedZ = ((.02 + p.u * (.27 + p.layerIndex * .055) + Math.pow(p.u, 2.3) * .17) * p.budHeight + (3 - p.layerIndex) * .026) * r;
          const openZ = Math.sin(p.u * Math.PI) * (.13 + p.layerIndex * .018) * r - p.u * p.u * p.layer.sag * r;
          x = radial * Math.cos(a); y = radial * Math.sin(a); z = closedZ + (openZ - closedZ) * easeBloom;
        }
        // Rotate the flower plane with the same camera orientation the user
        // controls, so it lives in the galaxy rather than facing the screen.
        const yaw = this.sceneYaw || 0;
        const cosYaw = Math.cos(yaw), sinYaw = Math.sin(yaw);
        const worldX = x * cosYaw - y * sinYaw;
        const worldY = x * sinYaw + y * cosYaw;
        const tilt = this.sceneTilt || Math.PI * .18;
        const yp = worldY * Math.sin(tilt) - z * Math.cos(tilt);
        const zp = worldY * Math.cos(tilt) + z * Math.sin(tilt);
        const perspective = 900 / (900 + zp), shimmer = .92 + Math.sin(p.phase) * .08;
        const gather = this.formationProgress;
        const useInitialGathering = this.state === 'GATHERING' && this.isFirstGathering;
        const targetMix = useInitialGathering ? gather : 1;
        let endX = cX + ((worldX * perspective) * targetMix + p.initialScatterX * (1 - targetMix)) * sceneScale;
        let endY = cY + ((yp * perspective) * targetMix + p.initialScatterY * (1 - targetMix)) * sceneScale;

        // During the former explosion stage, each particle makes a short,
        // irregular local orbit. The flower loosens and rearranges without
        // ever turning into a ring or travelling across the screen.
        if (this.state === 'DISPERSING') {
          const progress = Math.min(1, this.stateTimer / 5);
          const offset = Math.sin(progress * Math.PI) * p.rearrangeDistance * sceneScale;
          const angle = p.rearrangeAngle + progress * (1.6 + p.speed * .35);
          endX += Math.cos(angle) * offset;
          endY += Math.sin(angle) * offset * .76;
        }
        const alpha = (.42 + shimmer * .48) * (.2 + gather * .8);

        render.push({
          x: endX, y: endY, z: zp,
          size: Math.max(.2, p.baseSize * perspective * (.85 + shimmer * .3) * sceneScale),
          alpha: p.type === 'seed' ? alpha : alpha * (p.edge ? .62 : .78) * (.88 + p.u * .12),
          color: this.bloomProgress < .08 ? p.budColor : p.color,
          edge: p.edge,
          sparkle: false
        });
      });
  ...
  destroy() { cancelAnimationFrame(this.animationId); this.canvas.remove(); }
}
...
class GalaxyController {
  ...
  init() {
    ...
    this.sunflowerVisual = new SunflowerParticleVisual(this.container);
    this.sunflowerVisual.owner = this;
  ...
  createSolid3DParticleHeart() {
    ...
    this.heartTextSprites.push(sprite);
      // The heart still owns the original timeline and click hit-testing; only
      // its sprite rendering is replaced by the sunflower canvas.
  ...
  startTransition() {
    ...
    if (this.heartTextSprites) {
      this.heartTextSprites.forEach(s => { s.material.opacity = 0.0; s.visible = false; });
    }
    if (this.textSprites) {
  ...
  animate() {
    ...
    const shouldFrameBloom = this.sunflowerVisual && !this.sunflowerViewMoved &&
      (this.sunflowerVisual.state === 'BLOOMING' || this.sunflowerVisual.state === 'HOLDING');

    if (shouldFrameBloom) {
      // Automatically settle into the centered showcase view when the flower
      // opens. Any user drag, wheel, or touch movement releases this framing.
      this.targetRotation.x = 0.4;
      this.targetRotation.y = 0;
      this.rotationVel.x = 0;
      this.rotationVel.y = 0;
      this.targetZoom = 900;
    } else if (!this.isHeartClicked && !this.isMouseDown) {
    ...
    // Project the original heart anchor into the viewport so the replacement
    // stays in its exact scene position while the user rotates or zooms.
    if (this.sunflowerVisual) {
      // Keep the replacement flower steady at the original heart center;
      // its own clockwise rotation remains, but it does not bob vertically.
      // Sit just above the language-word orbit while remaining near the
      // centered showcase composition.
      const worldAnchor = new THREE.Vector3(0, 105, 0);
      const cameraDistance = this.camera.position.distanceTo(worldAnchor);
      const anchor = worldAnchor.project(this.camera);
      const x = (anchor.x * .5 + .5) * window.innerWidth;
      const y = (-anchor.y * .5 + .5) * window.innerHeight;
      const visible = this.stage >= 3 && !this.isExploding && !this.transitionTriggered && (!this.heartGroup || this.heartGroup.visible);
      // Use the same camera-distance relationship as the original 3D heart,
      // without inheriting its click-time heartbeat pulse.
      const sceneScale = Math.max(.45, Math.min(2.5, 1500 / cameraDistance));
      const sceneTilt = Math.max(.08, Math.min(Math.PI * .46, Math.PI * .18 + (this.currentRotation.x - .4) * .48));
      this.sunflowerVisual.setVisible(visible, x, y, sceneScale, this.currentRotation.y, sceneTilt);
    }
  ...
  onWindowResize() {
    ...
    this.renderer.setSize(window.innerWidth, window.innerHeight);
    this.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    if (this.sunflowerVisual) this.sunflowerVisual.resize();
  }

  destroy() {
    this.hideInfoBox();
    if (this.sunflowerVisual) {
      this.sunflowerVisual.destroy();
      this.sunflowerVisual = null;
    }
    if (this.animFrameId) {
      cancelAnimationFrame(this.animFrameId);
