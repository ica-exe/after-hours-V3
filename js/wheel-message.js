...
class WheelMessageController {
  constructor() {
    ...
    this.storageKey = 'ROMANTIC_WHEEL_MESSAGE_COLLECTION';
    ...
    this.modalBackdrop = document.getElementById('wheel-message-modal-backdrop');
    this.modalNumber = document.getElementById('wheel-message-card-number');
    this.modalVerse = document.getElementById('wheel-message-card-verse');
    this.modalStatusNotice = document.getElementById('wheel-message-modal-status-notice');
    this.modalActionBtn = document.getElementById('wheel-message-modal-action-btn');
    this.modalCloseBtn = document.getElementById('wheel-message-card-close');
    this.bookletBackdrop = document.getElementById('wheel-message-booklet-backdrop');
    ...
    this.bookletMessageNum = document.getElementById('booklet-message-num');
    this.bookletMessageText = document.getElementById('booklet-message-text');
    ...
    this.resetModalBackdrop = document.getElementById('wheel-message-reset-modal-backdrop');
  ...
  showResultModal(segIndex) {
    const messageNumber = segIndex + 1;
    const messageText = wheelMessages[segIndex] || wheelMessages[0];
    this.currentWinningNumber = messageNumber;
    this.currentWinningText = messageText;
    if (this.modalNumber) {
      this.modalNumber.textContent = `✦ NO. ${messageNumber.toString().padStart(2, '0')} ✦`;
    }
    if (this.modalVerse) {
      this.modalVerse.textContent = messageText;
    }
    const isAlreadyCollected = this.collectedNumbers.includes(messageNumber);
  ...
  renderBookletPage() {
    ...
    if (totalCollected === 0) {
      if (this.bookletMessageNum) {
        this.bookletMessageNum.textContent = "✦ YOUR COLLECTION IS WAITING ✦";
      }
      if (this.bookletMessageText) {
        this.bookletMessageText.textContent = "your collection is empty right now ♡\n\nspin the wheel to discover and collect messages for Elie.";
      }
    ...
    if (this.bookletMessageNum) {
      this.bookletMessageNum.textContent = `✦ NO. ${item.number.toString().padStart(2, '0')} ✦`;
    }
    if (this.bookletMessageText) {
      this.bookletMessageText.textContent = item.text;
    }
    if (this.bookletIndicator) {
